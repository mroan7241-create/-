...
const XLSX_INSTRUCTIONS_LINES = [
  ...
  '٥. عمود "الاحتياج" يقبل الأنواع الثلاثة المعتمدة فقط: ثلاجة، فرن، غسالة — يمكن كتابة أكثر من نوع في الخلية نفسها مفصولة بفاصلة عربية أو إنجليزية أو شرطة، مثل: ثلاجة، غسالة. أي نوع آخر (مثل مكيف أو فريزر أو سخان) يُرفض الصف كاملًا مع بيان رقم الصف والنوع غير المعتمد.',
