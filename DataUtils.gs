...
const ID_PREFIX_SOURCES_ = Object.freeze({
  ...
  // طور تصميم Phase 3.1 — لم يُطبَّق المخطط بعد ولا بيانات حية بهذه
  // البادئات، فالإضافة آمنة بالكامل بلا أي أثر على بيانات قائمة.
  RCB: {sheet: 'محاضر استلام الأجهزة', column: 'رقم المحضر'},
  RCI: {sheet: 'بنود محضر الاستلام', column: 'رقم البند'},
  RCD: {sheet: 'صور تلف الاستلام', column: 'رقم الصورة'}
});
 ...
 * تنفيذ fn() مرة أخرى.
 *
 * ⚠️ Phase 2.3.4 (القسم 7) — تسمية دقيقة، لا ادّعاء زائد: هذه **حماية
 * إعادة المحاولة المتزامنة وقصيرة المدى**، لا idempotency دائمة ولا
 * ضمان "exactly-once" مطلقًا. النتيجة تُخزَّن في CacheService لخمس
 * دقائق فقط (300 ثانية) — إعادة محاولة تصل بعد انتهاء هذه المهلة تُنفِّذ
 * fn() من جديد، وCacheService نفسها قد تُفرِغ القيمة قبل انتهاء المهلة
 * المعلَنة (بلا ضمان بقاء صارم من Google Apps Script). خط الدفاع الثاني
 * ضد التكرار الفعلي دائمًا هو فحوصات التكرار وقواعد الحالة نفسها
 * (findConfirmedDuplicateBeneficiary_، StateRules.gs، إلخ) — لا يجوز
 * الاعتماد على هذه الآلية وحدها لمنع تكرار حقيقي بعيد المدى. سجل idempotency
 * دائم (durable ledger على جدول/قاعدة بيانات مستقلة بدل الكاش المؤقت)
 * بند P1 مؤجَّل عمدًا حتى ما قبل النشر الإنتاجي الفعلي — راجع HANDOFF.md.
 *
