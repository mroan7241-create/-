...
function seedScenario(S, countPerAssociation) {
  ...
  for (let i = 0; i < countPerAssociation; i++) {
    S.saveBeneficiary(assocASession.token, { deviceTypes: ['ثلاجة'],
    ...
    S.saveBeneficiary(assocBSession.token, { deviceTypes: ['ثلاجة'],
...
section('2) استجابات الحفظ لا تُعيد getBootstrapData كاملًا');
{
  ...
  const saveResult = S.saveBeneficiary(assocASession.token, { deviceTypes: ['ثلاجة'],
