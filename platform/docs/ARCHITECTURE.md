...
## 4.2) NODE-2 — أول نطاق أعمال حقيقي (طلبات الانضمام + إدارة الجمعيات)

NODE-2 هو أول استخدام فعلي لثلاث لبنات كانت هيكلًا فارغًا حتى الآن:
**تخزين الكائنات**، **العدّاد العام الذرّي**، و**الـidempotency الدائم**.

### الحدود الجديدة بين الوحدات

- `ApplicationsModule` — يملك `association_applications` و
  `application_answers` وحده. يستهلك `FilesModule` (رفع/توقيع) و
  `AuditModule` و`PublicCodeService` و`IdempotencyService`.
- `AssociationsModule` — يملك `associations` وحده، ويكتب في `accounts`/
  `auth_credentials` **فقط** ضمن مسار الإنشاء المتعدي (قبول طلب أو إنشاء
  ADMIN مباشر)، بنفس المعاملة، ولا يقرأ من نطاقات أعمال أخرى إلا عبر
  عدّادات مجمَّعة للعرض.
- `FilesModule` — لم يعد هيكلًا: `StorageService` غلاف S3-متوافق حقيقي
  (MinIO محليًا/CI، أي مزوّد Production لاحقًا خلف نفس العقد بلا تغيير كود
  مستدعٍ). `files.controller.ts` العام حُذف — لا نقطة وصول عامة للملفات
  إطلاقًا، الوصول الوحيد عبر endpoint النطاق صاحب الملف (راجع
  SECURITY_MODEL.md).

### عمليات ACID الجديدة (تُضاف إلى §3)

| العملية | ما يجب أن يكون ذرّيًا معًا |
|---|---|
| تقديم طلب انضمام | `files` + عدّاد `APP` + `association_applications` + `application_answers` الثمانية |
| قبول طلب | ادّعاء idempotency + قفل صف الطلب + عدّاد `ASC` + `associations` + عدّاد `USR` + `accounts` + `auth_credentials` + تحديث الطلب + تسجيل نتيجة idempotency |
| إنشاء جمعية مباشر | ادّعاء idempotency + عدّاد `ASC` + `associations` + عدّاد `USR` + `accounts` + `auth_credentials` + تسجيل النتيجة |

### الحدّ الذي لا تستطيع المعاملة تغطيته: تخزين الكائنات

رفع الملف إلى S3 ليس جزءًا من معاملة PostgreSQL ولا يتراجع معها. النمط
المعتمد (ومطابق للقديم مع Drive): **رفع أولًا خارج المعاملة، ثم معاملة
قاعدة البيانات، ثم حذف تعويضي best-effort عند أي فشل**. `deleteObjectBestEffort`
لا يرمي أبدًا حتى لا يُخفي خطأ التنظيف الخطأ الأصلي. التفصيل الكامل في
ASSOCIATION_APPLICATIONS.md §4، ومُختبَر مقابل خادم S3 حقيقي لا mock.

### استبدال `LockService` بقيود قاعدة البيانات

القديم كان يسلسل كل كتابة عبر قفل سكربت واحد. المنصة الجديدة قد تعمل
على أكثر من instance، فبديل القفل هو:

- **فهارس فريدة جزئية** لقاعدة «طلب واحد قيد المراجعة لكل بريد/جوال/ترخيص»
  و«حساب `ASSOCIATION` تشغيلي واحد لكل جمعية».
- **`SELECT … FOR UPDATE`** لقفل صف الطلب أثناء المراجعة.
- **`INSERT … ON CONFLICT DO NOTHING`** لادّعاء الـidempotency ذرّيًا.
- **upsert-and-increment ذرّي** للعدّاد العام بدل `MAX(...)+1`.

كل واحدة منها يحرسها اختبار تزامن حقيقي (`Promise.all` على طلبات فعلية
ضد نفس التطبيق ونفس قاعدة البيانات)، لا محاكاة.
