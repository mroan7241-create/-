...
@Injectable() export class ReconciliationService{
 async reconcile(associationId:string){
  ...
  const movement=await prisma.$queryRaw<{id:string}[]>`SELECT du.id FROM device_units du LEFT JOIN device_movements dm ON dm.device_id=du.id WHERE du.association_id=${associationId}::uuid AND du.current_location_type<>'WAREHOUSE'::"DeviceMovementLocationType" GROUP BY du.id HAVING count(dm.id)=0`;
