...
export class ClosureService {
  ...
  async generate(ctx: AuthContext, participationId: string, opId: string) {
    const participation = await prisma.projectParticipation.findUnique({ where: { id: participationId }, select: { associationId: true } });
  ...
  async updateQualitative(ctx: AuthContext, id: string, fields: { challenges?: string; lessonsLearned?: string; recommendations?: string; finalNotes?: string }) {
    ...
    if (ctx.role !== AccountRole.ASSOCIATION || (report.status !== OrganizationClosureStatus.DRAFT && report.status !== OrganizationClosureStatus.GENERATED && report.status !== OrganizationClosureStatus.REOPENED)) throw new ApiError('CLOSURE_REPORT_LOCKED', 'التقرير مقفل في حالته الحالية', 409); return prisma.organizationClosureReport.update({ where: { id }, data: fields });
  }
  ...
  async transitionOrganization(ctx: AuthContext, id: string, to: OrganizationClosureStatus, opId: string) {
    ...
    const adminTransition = (current.status === OrganizationClosureStatus.SUBMITTED && to === OrganizationClosureStatus.UNDER_REVIEW) || (current.status === OrganizationClosureStatus.UNDER_REVIEW && to === OrganizationClosureStatus.APPROVED) || (current.status === OrganizationClosureStatus.APPROVED && to === OrganizationClosureStatus.CLOSED);
    if ((ctx.role === AccountRole.ASSOCIATION && !associationTransition) || (ctx.role === AccountRole.ADMIN && !adminTransition)) throw new ApiError('CLOSURE_TRANSITION_INVALID', 'انتقال تقرير الإغلاق غير مسموح', 409);
