...
import { activateParticipation, apiFetch, completeParticipationSetup, confirmPhysicalReturn, createAgreement, decideDelivery, decideEscalation, generateOrganizationClosure, listDeliveries, listEscalations, listNotifications, listParticipations, listProcurement, listSystemSettings, markNotificationRead, promoteReserve, reopenOrganizationClosure, saveSystemSetting, setBeneficiaryList, transitionAgreement, transitionOrganizationClosure, updateOrganizationClosure } from '../lib/api';
...
function Actions({ user, section, row, busy, act }: { user: CurrentUser; section: string; row: WorkflowRecord; busy: string; act: (label: string, action: () => Promise<unknown>) => Promise<void> }) {
  ...
  if (section === 'participations') { const agreement = (row.agreements as WorkflowRecord[] | undefined)?.[0]; const closure = row.closureReport as WorkflowRecord | undefined; const editClosure = () => updateOrganizationClosure(closure!.id, { challenges: window.prompt('التحديات') ?? '', lessonsLearned: window.prompt('الدروس المستفادة') ?? '', recommendations: window.prompt('التوصيات') ?? '', finalNotes: window.prompt('الملاحظات الختامية') ?? '' }); return user.role === 'ADMIN' ? <>{button('إنشاء اتفاقية', () => createAgreement(row.id, Number(window.prompt('رقم النسخة') ?? 1), window.prompt('نسخة القالب') ?? 'v1'))}{agreement && <span>الاتفاقية: {String(agreement.status)} {agreement.status === 'DRAFT' && button('إرسال الاتفاقية', () => transitionAgreement(agreement.id, 'SENT'))}{agreement.status === 'SENT' && button('توقيع الجمعية', () => transitionAgreement(agreement.id, 'SIGNED_BY_ORG', window.prompt('اسم موقّع الجمعية') ?? ''))}{agreement.status === 'SIGNED_BY_ORG' && button('توقيع زاد', () => transitionAgreement(agreement.id, 'SIGNED', window.prompt('اسم موقّع زاد') ?? ''))}</span>}{button('إكمال التجهيز', () => completeParticipationSetup(row.id))}{button('تفعيل', () => activateParticipation(row.id))}{closure?.status === 'SUBMITTED' && button('بدء المراجعة', () => transitionOrganizationClosure(closure.id, 'UNDER_REVIEW'))}{closure?.status === 'UNDER_REVIEW' && button('اعتماد الإغلاق', () => transitionOrganizationClosure(closure.id, 'APPROVED'))}{closure?.status === 'APPROVED' && button('إغلاق نهائي', () => transitionOrganizationClosure(closure.id, 'CLOSED'))}{closure?.status === 'CLOSED' && button('إعادة فتح', () => reopenOrganizationClosure(closure.id, window.prompt('سبب إعادة الفتح') ?? ''))}</> : <>{button('فحص جاهزية الإغلاق', () => apiFetch(`/reports/closure/readiness/${row.id}`))}{!closure && button('إنشاء تقرير إغلاق', () => generateOrganizationClosure(row.id))}{closure && <span>تقرير الإغلاق: {String(closure.status)} {(closure.status === 'GENERATED' || closure.status === 'REOPENED') && <>{button('تعديل المحتوى النوعي', editClosure)}{button('إرسال التقرير', () => transitionOrganizationClosure(closure.id, 'SUBMITTED'))}</>}</span>}</>; }
  ...
  if (section === 'deliveries' && row.status === 'PENDING_RETURN_APPROVAL' && user.role === 'ASSOCIATION') return <>{button('تأكيد عودة سليمة', () => confirmPhysicalReturn(row.id, 'GOOD', window.prompt('ملاحظات الاستلام الفعلي') ?? ''))}{button('تأكيد عودة تالفة', () => confirmPhysicalReturn(row.id, 'DAMAGED', window.prompt('ملاحظات التلف والاستلام الفعلي') ?? ''))}</>;
