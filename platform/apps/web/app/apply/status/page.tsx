...
export default function ApplicationStatusPage() {
  ...
  useEffect(() => {
    try { const saved = JSON.parse(localStorage.getItem(DRAFT_KEY) || 'null') as { draftCode?: string; resumeToken?: string } | null; if (saved?.draftCode && saved.resumeToken) { setDraftCode(saved.draftCode); setResumeToken(saved.resumeToken); } } catch { /* browser convenience only */ }
  ...
  return <main style={narrowPageStyle}>
    <h1 style={{ fontSize: 24, marginBottom: 8 }}>متابعة طلب انضمام جمعية</h1><p style={{ ...mutedStyle, marginTop: 0 }}>بيانات الاستئناف سرية وتمثل مفتاح الوصول إلى الطلب؛ لا تشاركها مع أي شخص.</p>
    ...
    <form onSubmit={lookup} style={{ ...cardStyle, display: 'grid', gap: 14, marginTop: 16 }}>
      {version === 'V2' ? <><label style={labelStyle}>رمز الطلب<input required value={draftCode} onChange={(e) => setDraftCode(e.target.value)} style={{ ...inputStyle, ...ltrStyle }} autoComplete="off" /></label><label style={labelStyle}>رمز الاستئناف السري<input required value={resumeToken} onChange={(e) => setResumeToken(e.target.value)} style={{ ...inputStyle, ...ltrStyle }} type="password" autoComplete="off" /></label></> : <label style={labelStyle}>معرّف المتابعة القديم<input required value={legacyId} onChange={(e) => setLegacyId(e.target.value)} style={{ ...inputStyle, ...ltrStyle }} /></label>}
      <button disabled={loading} style={primaryButtonStyle}>{loading ? 'جارٍ التحميل…' : 'عرض الحالة'}</button>{message && <p role="status" style={message.startsWith('تم') ? successStyle : errorStyle}>{message}</p>}
    </form>
