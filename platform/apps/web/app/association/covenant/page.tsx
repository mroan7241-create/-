...
export default function AssociationCovenantPage() {
  ...
  const [covenantLoading, setCovenantLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    void getOwnCovenant()
      .then((value) => {
        setCovenant(value);
        setRepresentativeName(value.representativeName ?? '');
        setRepresentativeTitle(value.representativeTitle ?? '');
      })
      .catch((reason) => { setCovenant(null); setError(readError(reason)); })
      .finally(() => setCovenantLoading(false));
  }, [user]);
